...
static void TestCD_CMSSignAndVerify(nlTestSuite * inSuite, void * inContext)
{
    ...
    memcpy(importableKeypair, sTestCMS_SignerSerializedKeypair, sizeof(sTestCMS_SignerSerializedKeypair));
    importableKeypair.SetLength(sizeof(sTestCMS_SignerSerializedKeypair));
    cdContentIn   = ByteSpan(sTestCMS_CDContent02);
    signedMessage = MutableByteSpan(signedMessageBuf);
    NL_TEST_ASSERT(inSuite, keypair2.Import(importableKeypair) == CHIP_NO_ERROR);
